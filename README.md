...
class Login extends React.Component {
    state = {
        height: window.innerHeight, // 浏览器高度, innerHeight网页内容实际高度
    };

    render () {
        return ( // LoginWrapper的height和LoginContainer的margin-top实现联动
    ...
    componentDidMount(){
        ...
        window.onresize = function(){
            if(window.innerHeight > 600){ // 当浏览器页面高度 < 600, 不再变化
